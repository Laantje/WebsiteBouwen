...
$naam = "persoon1";
			...
			$username = $firstname = $lastname = $email = $adress = $postalcode = $city = $phone =  "";
				...
				$query = "SELECT username, firstname, lastname, email, adress, postalcode, city, phone FROM user WHERE username ='". $_SESSION["editUser"] . "'";
...
//testen of de velden zijn ingevuld
	...
	if ($_SERVER["REQUEST_METHOD"]=="POST" AND !empty($username_new) AND !empty($firstname_new) AND !empty($lastname_new) AND !empty($email_new) AND !empty($adress_new) AND !empty($postalcode_new) AND !empty($city_new) AND !empty($phone_new) AND !$hasError){
        $update="UPDATE user SET username = '$username_new', firstname = '$firstname_new', lastname = '$lastname_new', email ='$email_new', adress ='$adress_new', postalcode = '$postalcode_new', city = '$city_new', phone = '$phone_new' WHERE username = '". $_SESSION["editUser"] . "'";
